...
define(['app',
        ...
        'safe/gis/lib/supermap/8.1.0/SuperMap.Include',
        'safe/gis/js/services/safeGisMapService'
    ],
    function (app) {
    app.directive('safeGisMap', ['safeGisMapService', '$interval', function(safeGisMapService, $interval) {
        ...
        return {
            ...
            replace: true,
            template: getTemplate($.soa.getWebPath('iiw.safe.gis') + '/view/map.html'),
            compile: function() {
                return {
                    post: function($scope, $element) {
                        ...
                        //加载地图类库
                        loadSuperMapLibs(initMap);

                        //初始化地图
                        function initMap() {
                            window.setTimeout(function() {
                                safeGisMapService.init(mapLoadComplete, $scope.toggle.isOverviewMap);
                            }, 1000);
                        }
                        ...
                        $scope.menuClick = function(row) {
                            switch(row.type) {
                                case 'dtbz':
                                    safeGisMapService.drawPoint();
                                    break;
                                case 'kx':
                                    safeGisMapService.drawRectangle();
                                    break;
                                case 'tsbh':
                                    $scope.showPlotInfo();
                                    break;
                                case 'ddcx':
                                    ...
                                    break;
                                case 'hcqfx':
                                    safeGisMapService.distanceMeasure(true);
                                    break;
                                case 'qc':
                                    $scope.clearMap();
                                    break;
                        ...
                        $scope.barClick = function(bar) {
                            switch(bar.type) {
                                case 'zoomin':
                                    safeGisMapService.zoomIn();
                                    break;
                                case 'zoomout':
                                    safeGisMapService.zoomOut();
                                    break;
                                case 'eagle':
                                    safeGisMapService.showEagle();
                                    break;
                                case 'distance':
                                    safeGisMapService.distanceMeasure(false);
                                    break;
                                case 'measure':
                                    safeGisMapService.areaMeasure();
                                    break;
                                default:
                                    break;
                            }
                        ...
                        $scope.clearFeatures = function() {
                            safeGisMapService.clearFeatures();
                        };

                        //编辑要素
                        $scope.editFeatures = function() {
                            safeGisMapService.addData();
                        };
                        ...
                        function initPlotInfo() {
                            ...
                            var symbolLibManager = safeGisMapService.getPlotData();
                        ...
                        $scope.startPloting = function(item) {
                            safeGisMapService.startPloting(item);
                        };
                        ...
                        $scope.drawTaskArea = function() {
                            safeGisMapService.drawTaskArea();
                            //safeGisMapService.showTaskArea();
                        };

                        $scope.showLocation = function() {
                            safeGisMapService.realTimeTrajectory();
                        };

                        $scope.start = function() {
                            safeGisMapService.startPlay();
                        };
                        ...
                        $scope.mapSearch = function() {
                            $scope.placeList = [];
                            safeGisMapService.getFeaturesBySQL($scope.search.name, showPlaceList);
                        };
                        ...
                        $scope.moveToThisPlace = function(row) {
                            ...
                            safeGisMapService.setMapCenter(row.data.SMX, row.data.SMY);
                        };
                                ...
                                $scope.intervalFlag = $interval(function() {
                                    if($scope.mapLoaded) {
                                        ...
                                        if(data.longitude && data.latitude) {
                                            safeGisMapService.showAlarmPoint(data);
                                        } else {
                                            var params = {
                                                longitude: 0,
                                                latitude: 0
                                            };
                                            safeGisMapService.showAlarmPoint(params);
                                        }
                                    }
                                }, 1000);
                            }
