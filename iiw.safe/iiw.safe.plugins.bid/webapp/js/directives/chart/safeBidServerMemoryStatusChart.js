...
define([
    ...
    app.directive('safeBidServerMemoryStatusChart', ['$interval', '$filter', 'iTimeNow', 'iAjax', function($interval, $filter, iTimeNow, iAjax){
        return {
            restrict: 'AE',
            replace: true,
            scope: true,
            link: function($scope, $element) {
                var childEle = $element.find('.safe-bid-server-memory-status-chart-panel').get(0);
                ...
                var interval = $interval(function() {
                    iAjax.post('security/homepage.do?action=getMemoryInformation').then(function(data) {
                        if (data.result && data.result.rows) {
                            $scope.memory = data.result.rows[0];
                            var axisData = $filter('date')(iTimeNow.now, 'HH:mm:ss');
                            xAxis.shift();
                            xAxis.push(axisData);
                            datas.shift();
                            datas.push($scope.memory.value[0]);
                            _chart.setOption({
                                xAxis: [
                                    {data: xAxis}
                                ],
