...
define([
    ...
    app.factory('safeDesService', [function() {

        /**
         * 随机取字符
         */
        function _randomChar(l) {
         ...
         * 加密字符串
         */
        function _strEnc(data, firstKey, secondKey, thirdKey) {
            ...
            try {
                l = encData.length / 2;
                AsciiVal = ( encData.substring(l, l + 1) ).charCodeAt();
                AsciiVal--;
                if (AsciiVal == 47)  AsciiVal = 57;
                if (AsciiVal == 64)  AsciiVal = 90;
                if (AsciiVal == 96)  AsciiVal = 122;
                if (AsciiVal == 58)  AsciiVal = 48;
                if (AsciiVal == 91)  AsciiVal = 65;
                if (AsciiVal == 123) AsciiVal = 97;
                encData = encData.substring(0, l) + String.fromCharCode(AsciiVal).toUpperCase() + encData.substring(l + 1, encData.length);
            } catch (e) {
                console.log(e);
            }
        ...
        function _strDec(data, firstKey, secondKey, thirdKey) {
            var leng = data.length;
            var decStr = '';
            var firstKeyBt, secondKeyBt, thirdKeyBt, firstLength, secondLength, thirdLength;
            try {
                l = data.length / 2;
                AsciiVal = ( data.substring(l, l + 1) ).charCodeAt();
                AsciiVal++;
                if (AsciiVal == 47)  AsciiVal = 57;
                if (AsciiVal == 64)  AsciiVal = 90;
                if (AsciiVal == 96)  AsciiVal = 122;
                ...
                if (AsciiVal == 91)  AsciiVal = 65;
                if (AsciiVal == 123) AsciiVal = 97;
                data = data.substring(0, l) + String.fromCharCode(AsciiVal).toUpperCase() + data.substring(l + 1, data.length);
            } catch (e) {
                console.log(e);
            }
        ...
        function _byteToString(byteData) {
            var str = '';
            for (i = 0; i < 4; i++) {
                var count = 0;
                for (j = 0; j < 16; j++) {
        ...
        function _enc(dataByte, keyByte) {
        ...
        function _dec(dataByte, keyByte) {
            var keys = this.generateKeys(keyByte);
            var ipByte = this.initPermute(dataByte);
            var ipLeft = new Array(32);
            var ipRight = new Array(32);
        ...
        function _initPermute(originalData) {
        ...
        function _expandPermute(rightData) {
        ...
        function _sBoxPermute(expandByte) {
        ...
        function _finallyPermute(endByte) {
        ...
        function _generateKeys(keyByte) {
        ...
        return {
            ...
            strEnc: _strEnc,
            strDec: _strDec,
            getKeyBytes: _getKeyBytes,
            strToBt: _strToBt,
            bt4ToHex: _bt4ToHex,
            hexToBt4: _hexToBt4,
            hexToBt64: _hexToBt64,
            byteToString: _byteToString,
            bt64ToHex: _bt64ToHex,
            enc: _enc,
            dec: _dec,
            initPermute: _initPermute,
            expandPermute: _expandPermute,
            xor: _xor,
            sBoxPermute: _sBoxPermute,
            pPermute: _pPermute,
            finallyPermute: _finallyPermute,
            getBoxBinary: _getBoxBinary,
