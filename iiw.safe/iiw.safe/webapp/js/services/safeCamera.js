...
define([
     ...
     * @Date : 2015-11-10
     */
    app.factory('safeCamera', ['$rootScope', '$compile', 'iAjax', 'iTimeNow', 'iMessage', 'safePolling', '$filter', 'safeVideoObject', 'safeHardware', 'safeImcsPlayer', function($rootScope, $compile, iAjax, iTimeNow, iMessage, safePolling, $filter, safeVideoObject, safeHardware, safeImcsPlayer) {
        ...
        function Camera(config) {
            ...
            function _record(id, index, start, end, now, errorCb, statusCb) {
                ...
                if(safeImcsPlayer.isconnect()) {
                    $scope.imcsPlayer.record(id, index, start, end, now, errorCb);
                    var o = $scope.getObject(index);
            ...
            function _recordstopall() {
                if(safeImcsPlayer.isconnect()) {
                    $scope.imcsPlayer.recordStopAll();
                    return;
                }
