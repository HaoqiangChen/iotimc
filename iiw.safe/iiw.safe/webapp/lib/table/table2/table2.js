...
layui.define(["jquery"], function (exports, undef) {
    ...
    function exportTable(name) {
        name = name ? name + '.xlsx' : '导出.xlsx';
        var mergoParams = [];

        var sheet = csvToSheet(cvs(this, mergoParams));

        if(mergoParams.length) {
            sheet['!merges'] = mergeGird(mergoParams);
        }

        openDownloadDialog(sheet2blob(sheet), name);
    }

    function mergeGird(params) {
        ...
        if(params && params.length) {
            ...
            params.map(function(row) {
                if(rowPlace[row.row] == null) {
                    rowPlace[row.row] = 0;
                }

                message.push({
                    s: {r: row.row, c: rowPlace[row.row]},
                    e: {r: row.row, c: rowPlace[row.row] + row.number - 1}
                });

                rowPlace[row.row] += row.number;
            });
    ...
    function cvs(obj, params) {
        ...
        var rows = [];
        if(obj.options.cols && obj.options.cols.length == 1) {

            obj.options.cols[0].map(function(row) {
                rows.push(row.title);
                names.push(row.field);
            });
            result.push(rows.join(','));
        }
        else if(obj.options.cols && obj.options.cols.length > 1) {
            var size = obj.options.cols[obj.options.cols.length - 1].length;
            obj.options.cols.map(function(objCols, iRow) {

                names = [];
                rows = new Array(size);
                var num = 0;
                objCols.map(function(objCol, iCol) {
                    names.push(objCol.field);
                    rows[num] = objCol.title;
                    if(objCol.colspan && objCol.colspan >= 2) {
                        num += objCol.colspan;
                        params.push({
                            row: iRow,
                            col: iCol,
                            number: objCol.colspan
                        });
                    }
                    else {
                        num++;
