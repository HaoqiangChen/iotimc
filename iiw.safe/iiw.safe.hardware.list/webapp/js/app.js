...
define([
  ...
  app.controller('hardwareListController', [
    ...
    function ($scope, $state, $filter, iAjax, safeMapSelect, safeMainTitle, safeGlobalSearch, safeHardware, iMessage, iConfirm) {

      if (!$scope.$parent.active) {
        $state.go('safe.hardware');
        return;
      } else {
        $scope.hardwareType = $scope.$parent.active.type;
      }
      ...
      $scope.getList = function () {
        ...
        // if ($scope.hardwareType === 'door' || $scope.hardwareType === 'talk') {
        //   delete deviceParams.params;
        // }
        iAjax.post('security/device/device.do?action=getDeviceActionOuList', deviceParams).then(function (data) {
