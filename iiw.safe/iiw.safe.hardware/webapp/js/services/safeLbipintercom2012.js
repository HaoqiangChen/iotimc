...
define([
    ...
    app.factory('safeLbipintercom2012', ['$timeout', 'iAjax', 'iMessage', 'safeHardware', 'safeSound', function($timeout, iAjax, iMessage, safeHardware, safeSound) {
        var _type = 'lonbon',
            _object,
            _serverip,
            _scope,
        ...
        function talk(source, target){
            ...
            if(_object) {
                _object.Exec('lb_fun_call', JSON.stringify({
        ...
        function hangup(source, target){
            if(_object) {
                _object.Exec('lb_fun_hungup', JSON.stringify({
                    displayNum: target.devnum,
                    areaId: 0,
                    devRegType: 0
            ...
            if(window.__IIWHOST) {
                init(function(row) {
                    ...
                    if(res == 0) {
                        ...
                        _object.Exec('lb_ipIntercom_start', JSON.stringify({
        ...
        function _unload() {
            if(_object) {
                ...
                //释放
                _object.Exec('lb_ipIntercom_stop', JSON.stringify({
