...
define([
    'app',
    'angularAMD',
    'cssloader!system/preplan/css/index.css',
    'system/preplan/js/directives/systemPreplanDeviceDialogDirective'
], function(app, angularAMD) {
    var packageName = 'iiw.system.preplan';

    app.controller('preplanController', ['$scope', '$state', 'iAjax', 'iTimeNow', 'mainService', 'iMessage', function($scope, $state, iAjax, iTimeNow, mainService, iMessage) {
         ...
         * @Date : 2016-03-19
         */
        $scope.$on('preplanControllerOnEvent', function() {
            $scope.init();
        });
        ...
        function getBroadAction() {
            ...
            iAjax.post('security/device/device.do?action=getTypeActionList', data).then(function(data) {
                if (data.result && data.result.rows) {
                    $scope.broadAction = data.result.rows[0].actions;
                } else {
                    $scope.broadAction = [];
                }
            }, function() {
    ...
    app.controller('preplanItemController', ['$scope', '$state', 'iAjax', 'iTimeNow', 'mainService', 'iMessage', '$filter', function($scope, $state, iAjax, iTimeNow, mainService, iMessage, $filter) {
        $scope.wizardIndex = 1;
    ...
    angularAMD.config(['$stateProvider', function($stateProvider) {
        $stateProvider.state('system.preplan.mod', {
            url: '/mod',
            templateUrl: $.soa.getWebPath(packageName) + '/view/add.html',
            controller: 'preplanItemController'
        });
    }]);
});